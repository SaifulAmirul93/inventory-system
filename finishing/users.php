...
<body>
	<div id="main-wrapper">
		...
		<div class="wrapper-pad">
			<?php
			if($s != false)
				echo '<h2>Users - Search</h2>';
			else
				echo '<h2>Users</h2>';
			?>
			<div id="table-head">
				<form method="post" action="" name="searchf">
					<input type="text" name="search" placeholder="Search..." class="search fleft" />
				</form>
				<div class="fright">
...
?>
					<tr data-type="element" data-id="<?php echo $user->id; ?>">
						...
						<td>
							<?php
							if($role == 1 || $role == 2)
								echo '<a href="edit-user.php?userid='.$user->id.'" name="c3" title="Edit User"><i class="fa fa-pencil"></i></a>';
							?>
							<a href="logs.php?userid=<?php echo $user->id; ?>" name="c4" title="Log of this user"><i class="fa fa-file-text-o"></i></a>
							<?php
							if($role == 1 || $role == 2)
								echo '<a href="" name="c5" title="Delete User"><i class="fa fa-close"></i></a>';
							?>
						</td>
					</tr>
<?php
