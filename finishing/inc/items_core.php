...
class Items {
	...
	public function get_items($ejuice,$page, $items_per_page) {
		$ejuice = stripslashes($ejuice);
		...
		$res = $this->query("SELECT * FROM invento_items WHERE ejuice_type = $ejuice ORDER BY id DESC LIMIT $x,$y", 'get_items()');
		return $res;
	}
	...
	public function count_items_finished() {
		$res = $this->query("SELECT SUM(qty) AS s FROM invento_items WHERE category = 1", 'count_items_finished()');
		$obj = $res->fetch_object();
		return $obj->s;
	}
	public function count_items_unfinished() {
		$res = $this->query("SELECT SUM(qty) AS s FROM invento_items WHERE category = 2", 'count_items_unfinished()');
		$obj = $res->fetch_object();
		return $obj->s;
	}
	...
	public function new_item($name, $desc, $cat, $qty, $price, $ejuices) {
		$name = stripslashes($name);
		$desc = stripslashes($desc);
		$cat = stripslashes($cat);
		$qty = stripslashes($qty);
		$price = stripslashes($price);
		$ejuices = stripslashes($ejuices);
		$date = date('Y-m-d');
		if($qty == '')
			$qty = 0;
			
		$prepared = $this->prepare("INSERT INTO invento_items(name,descrp,category,qty,price,date_added,ejuice_type) VALUES(?,?,?,?,?,?,?)", 'new_item()');
		$this->bind_param($prepared->bind_param('ssiiisi', $name, $desc, $cat, $qty, $price, $date, $ejuices), 'new_item()');
