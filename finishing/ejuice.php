...
if(isset($_POST['act'])) {
	...
	if($_POST['act'] == '1') {
		...
		if($_ejuice->count_ejuice_search($search_string) == 0)
			die('2');
		die('3');
	}
	...
	if($_POST['act'] == '2') {
		if(!isset($_POST['id']) || $_POST['id'] == '')
			die('wrong');
		if($_ejuice->delete_ejuice($_POST['id']) == true)
			die('1');
		die('wrong');
	}
}
...
if(isset($_GET['search']) && ($_GET['search'] != '')){
	...
	$c_cats = $_cats->count_ejuice_search($s);
}else{
	$s = false;
	$ejuices = $_ejuice->get_ejuices($page, $pp);
	$c_ejuice = $_ejuice->count_ejuice();
}
?>
	...
	<div id="main-wrapper">
		...
		<div class="wrapper-pad">
			...
			<div id="table-head">
				...
				<?php
				if($role == 1 || $role == 2)
					echo '<a href="new-ejuice.php" name="new-cat" class="btn blue fright"><i class="fa fa-plus"></i>New Lineup</a>';
				?>
			</div>
			...
			<table border="1" rules="rows" id="categories">
				...
				<tbody>
<?php
					if($_ejuice->is_mysqlnd() || $s == false) {
						while($ejuice = $ejuices->fetch_object()) {
?>
					<tr data-type="element" data-id="<?php echo $ejuice->id;?>">
						<td><input type="checkbox" name="chbox" /></td>
						<td class="hover" data-type="id"><?php echo $ejuice->id; ?></td>
						<td class="hover" data-type="name"><?php echo $ejuice->name; ?></td>
						<td></td>
						<td>0<!-- <?php echo $_items->get_cat_reg_items($cat->id); ?> --></td>
						<td>0<!-- <?php echo $_items->get_cat_tot_items($cat->id); ?> --></td>
						<td>
							<?php
							if($role == 1 || $role == 2)
								echo '<a href="edit-ejuice.php?id='.$ejuice->id.'" name="c3" title="Edit Item"><i class="fa fa-pencil"></i></a>';
							if($role == 1 || $role == 2 || $role == 3)
								echo '<a href="logs.php?catid='.$ejuice->id.'" name="c4" title="Log of this category"><i class="fa fa-file-text-o"></i></a>';
							if($role == 1 || $role == 2)
...
<div id="pagination">
			...
			<?php
			if($s == false)
				$total_items = $_ejuice->count_ejuice();
			else
				$total_items = $_ejuice->count_ejuice_search($s);
			if($total_items > $pp) {
