...
$('document').ready(function() {
	...
	$('form[name=searchform]').submit(function(evt) {
		evt.preventDefault();
		$('.loader').fadeIn(200);
		var val_cat = $(this).children('input[name=item-category]').val();
		var val_item = $(this).children('input[name=item-name]').val();
		var val_username = $(this).children('input[name=user-name]').val();
		var val_date = $(this).children('input[name=date]').val();
		if((val_cat == '')&&(val_item == '')&&(val_username == '')&&(val_date == '') ) {
			$('.loader').fadeOut(200);
			alert('Please write your search');
			return false;
		}
