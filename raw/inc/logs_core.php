...
//		
		...
		elseif((($type == false)||($user_id == false)||($date == false)) && ($item_id != false ) ){
	
			$prepared = $this->prepare("SELECT * FROM invento_logs WHERE item LIKE ? " , 'search_filter()');
			$this->bind_param($prepared->bind_param('i', $item_id), 'search_filter()');
		}
